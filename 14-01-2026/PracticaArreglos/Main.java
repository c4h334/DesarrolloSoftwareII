...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        IOManager.write("EJERCICIO 6");
        int[] arrayToInterchange = {2, 4, 6};
        IOManager.write("The original array was " + printArray(arrayToInterchange) + "\nand the one with the first and last values interchanged is " + printArray(interchangeValues(arrayToInterchange)));

        // Cree un nuevo arreglo que contenga los elementos de dos arreglos.
        IOManager.write("EJERCICIO 7");
        int[] array1 = {1, 2, 3};
        int[] array2 = {4, 5, 6};
        IOManager.write("The first array is " + printArray(array1) + " and the second one " + printArray(array2) + "\nThe new array formed by these two is " + printArray(twoArraysInOne(array1, array2)));

        /* --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
        // Cree un arreglo de objetos (por ejemplo, Persona) y muestre sus datos.
        IOManager.write("EJERCICIO 1");
        Person person1 = new Person("Martha", 21);
        Person person2 = new Person("Jonas", 20);
        Person person3 = new Person("Anna", 18);
        Person person4 = new Person("Louis", 19);

        Person[] peopleArray = {person1, person2, person3, person4};
        for (int i = 0; i < peopleArray.length; i++) {
            IOManager.write(peopleArray[i].toString());
        }

        // Dado un arreglo de objetos, busque un objeto por un atributo específico.
        IOManager.write("\nEJERCICIO 2");
        IOManager.write("What's the name you wish to find?");
        Object nameToFind = IOManager.read();

        for (Person person : peopleArray) {
            if (person.name.equals(nameToFind)) {
                IOManager.write("Person found: " + person.toString());
            }
    ...
    static int[] interchangeValues(int[] array) {
        int temp = 0;
        temp = array[0];
